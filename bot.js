...
xt}". Usando 0 segundos.`);
  ...
  function timeToMilliseconds(timeObj) {
    return (timeObj.hours * 3600 + timeObj.minutes * 60 + timeObj.seconds) * 100
0;
  }
        ...
        await page.type('input[placeholder="Please enter password"]', password, 
{
      ...
      const selectorGift = "#__nuxt > div.ucenter-header > div.header > div > di
v.flow-box > img";
      ...
      try {
        ...
        throw new Error(`No se pudo hacer clic en el elemento del premio: ${e.me
ssage}`);
      }
      ...
      const confirmButtonSelector = "body > div.dialog-flow-box > div > div.butt
on";
      let prizeClaimed = false;
      
      try {
        await page.waitForSelector(confirmButtonSelector, { timeout: 5000 });
        console.log("✅ Botón de confirmación encontrado. Haciendo segundo clic 
...
;
        ...
        await page.waitForSelector('div.money span', { timeout: 15000 });
        const newBalance = await page.$eval('div.money span', el => el.textConte
nt);
        
        const newTime = getCurrentTime();
        if (newBalance !== balance) {
          console.log(`🎉 Balance incrementado a las ${newTime} : ${balance} → $
{newBalance}`);
        } else {
          console.log(`ℹ️ Balance sin cambios a las ${newTime} : ${balance} → ${n
ewBalance}`);
        }
        ...
        try {
          // Hacer clic nuevamente en el elemento del premio para ver el nuevo c
onteo
...
");
          ...
          const countdownText = await page.$eval('div.time', el => el.textConten
t);
...
close-button";
 ...
 normal).");
          ...
          console.log("⚠️ No se pudo obtener el nuevo conteo regresivo. Reintenta
ndo en 5 minutos...");
          setTimeout(runCycle, 300000); // 5 minutos
        }
        
      } catch (confirmButtonError) {
        // Si no se encuentra el botón de confirmación, verificar si hay conteo 
regresivo
        console.log("ℹ️ No se encontró botón de confirmación. Verificando si hay 
conteo regresivo...");
...
close-button";
 ...
 normal).");
          ...
          console.log("⚠️ No se encontró ni botón de confirmación ni conteo regre
sivo. Reintentando en 5 minutos...");
          setTimeout(runCycle, 300000); // 5 minutos
        }
      }
...
root@debdocker:/opt/docker/packetshare# [689637.437623] br-e6ebdd515545: port 1(veth28
root@debdocker:/opt/docker/packetshare# more bot.js
// bot.js
const puppeteer = require("puppeteer");
